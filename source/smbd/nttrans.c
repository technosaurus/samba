				  ...
				  char **ppdata, uint32 data_count, uint32 max_data_count)
{
	...
	case FSCTL_FIND_FILES_BY_SID: /* I hope this name is right */
	{
		...
		if (!sid_parse(pdata+4,sid_len,&sid)) return ERROR_NT(NT_STATUS_INVALID_PARAMETER);
				  ...
				  char **ppdata, uint32 data_count, uint32 max_data_count)
{
	...
	switch (level) {
		...
		case TRANSACT_GET_USER_QUOTA_FOR_SID:
			...
			if (!sid_parse(pdata+8,sid_len,&sid)) return ERROR_NT(NT_STATUS_INVALID_PARAMETER);
				  ...
				  char **ppdata, uint32 data_count, uint32 max_data_count)
{
	...
	if (!sid_parse(pdata+40,sid_len,&sid)) return ERROR_NT(NT_STATUS_INVALID_PARAMETER);
	DEBUGADD(8,("SID: %s\n",sid_string_static(&sid)));
